...
#
# The baseline is the last commit of the imported Flutter app, whose history
# carries a hostname from before this constitution existed. See ADR 0002.
set -uo pipefail
cd "$(dirname "$0")/.."
...
history_hits="$(git rev-list --all --not "$baseline" | xargs -n 50 -r git grep -lIi -f "$pat" 2>/dev/null | sort -u)"
