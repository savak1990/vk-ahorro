...
**Depends on:** 000-constitution, 010-repo-bootstrap
...
- With `AUTH_DISABLED` unset and the lab pool values: `curl` without a token → 401; with `aws cognito-idp initiate-auth` id token → 200 with the user's email.
