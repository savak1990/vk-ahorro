...
**Depends on:** 070-flutter-shell-trim, 050-terraform-cognito, 020-go-hello-service
...
Excludes: the Cognito pool (050), the ConfigMap that serves `config.json`
...
4. `flutter-ui/web/config.json` MUST be committed with local defaults: `apiBaseUrl: http://localhost:8080`, empty Cognito values. `flutter-ui/config/*.json` MUST be gitignored; `make ui-config ENV=lab FQDN=<fqdn>` writes `flutter-ui/config/lab.json` from `make tf-outputs` and sets `apiBaseUrl` to `https://api-ahorro.<fqdn>`.
...
- Lab, each platform: sign up and sign in with the Authenticator against the pool from 050, tap "+", see "Hello, <email>"; the Go pod log shows the same `X-Request-Id` the app logged.
